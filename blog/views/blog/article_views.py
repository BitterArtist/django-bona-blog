...
class ArticleDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["related_articles"] = Article.objects.filter(
            category=self.object.category, status=Article.PUBLISHED
        ).order_by("?")[:3]
    ...
    def get_queryset(self):
        ...
        if query:
            ...
            search_results = Article.objects.filter(
                ...
                | reduce(operator.and_, (Q(body__icontains=q) for q in query_list))
            )
    ...
    def get_queryset(self):
        ...
        if tag_name:
            tag_articles_list = Article.objects.filter(
                tags__name__in=[tag_name], status=Article.PUBLISHED, deleted=False
            )
