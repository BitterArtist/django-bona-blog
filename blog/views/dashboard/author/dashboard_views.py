    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == self.PUBLISH:
            ...
            if article_create_form.is_valid():
                ...
                return redirect(
                    to="blog:dashboard_article_detail", slug=new_article.slug
                )
...
class ArticleUpdateView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        article_update_form = ArticleUpdateForm(
            instance=old_article, initial={"tags": old_article.tags.names}
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == self.PUBLISH:
            ...
            if article_update_form.is_valid():
                ...
                return redirect(
                    to="blog:dashboard_article_detail", slug=updated_article.slug
                )
...
class AuthorWrittenArticlesView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        written_articles = Article.objects.filter(
            author=request.user.id, deleted=False
...
class AuthorPublishedArticlesView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        published_articles = Article.objects.filter(
            author=request.user.id, status=Article.PUBLISHED, deleted=False
...
class AuthorDraftedArticlesView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        drafted_articles = Article.objects.filter(
            author=request.user.id, status=Article.DRAFTED, deleted=False
        ).order_by("-date_created")
...
class AuthorDeletedArticlesView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        deleted_articles = Article.objects.filter(
            author=request.user.id, deleted=True
        ).order_by("-date_published")
