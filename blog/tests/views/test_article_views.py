...
class ArticleListViewTestCase(TestCase):
    ...
    def test_if_article_list_view_returns_the_right_category_details(self):
        ...
        self.assertEqual(
            response.context_data["categories"][0].name, self.category.name
        )
        self.assertEqual(
            response.context_data["categories"][0].slug, self.category.slug
        )
        self.assertEqual(
            response.context_data["categories"][0].image, self.category.image
        )
    ...
    def test_if_article_list_view_returns_the_right_article_details(self):
        ...
        self.assertEqual(
            response.context_data["articles"][0].category, self.articles[3].category
        )
        self.assertEqual(
            response.context_data["articles"][0].title, self.articles[3].title
        )
        self.assertEqual(
            response.context_data["articles"][0].slug, self.articles[3].slug
        )
        self.assertEqual(
            response.context_data["articles"][0].author, self.articles[3].author
        )
        self.assertEqual(
            response.context_data["articles"][0].image, self.articles[3].image
        )
        self.assertEqual(
            response.context_data["articles"][0].body, self.articles[3].body
        )
        self.assertEqual(
            response.context_data["articles"][0].date_published,
            self.articles[3].date_published,
        )
        self.assertEqual(
            response.context_data["articles"][0].date_created,
            self.articles[3].date_created,
        )
        self.assertEqual(
            response.context_data["articles"][0].status, self.articles[3].status
        )
...
class ArticleDetailViewTestCase(TestCase):
    ...
    def test_if_article_detail_view_returns_the_right_article_details(self):
        response = self.client.get(self.article.get_absolute_url())
        self.assertEqual(response.context["article"].category, self.article.category)
        ...
        self.assertEqual(response.context["article"].author, self.article.author)
        ...
        self.assertEqual(
            response.context["article"].date_published, self.article.date_published
        )
        self.assertEqual(
            response.context["article"].date_created, self.article.date_created
        )
        self.assertEqual(response.context["article"].status, self.article.status)
...
class ArticleSearchListViewTestCase(TestCase):
    def setUp(self):
        self.client = Client()
        self.articles = mommy.make(
            Article, _quantity=5, body="Test", status="PUBLISHED"
        )
    ...
    def test_article_search_list_view_returns_the_right_query_results(self):
        ...
        self.assertEqual(
            response.context["search_results"][0].slug, self.articles[0].slug
        )
